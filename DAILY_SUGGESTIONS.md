...
- **종목별 가격 경보 (Price Alert) 기능** (`static/js/app.js` 관심종목 탭, `localStorage`, `loadMarket()` 폴링 루프): 학생이 특정 종목의 목표가를 미리 설정해두면 주가가 해당 금액 이상/이하가 될 때 토스트 알림으로 알려주는 기능이 없어, 실제 HTS/MTS의 핵심 기능인 "목표가 알림"을 체험할 수 없음. `localStorage.setItem('alerts', JSON.stringify({[symbol]: {above: 숫자, below: 숫자}}))` 형태로 알림 조건 저장. `loadMarket()` 루프(`app.js:1357` 근처 `S.stocks` 업데이트 직후)에서 `Object.entries(alerts).forEach(([sym, cond]) => { const cur = S.stocks.find(s => s.symbol === sym)?.price; if (cond.above && cur >= cond.above) toast(...); })` 약 10줄 추가. 종목 상세 모달에 "🔔 알림 설정" 미니 폼(목표가 이상/이하 + 삭제 버튼) 추가. `app.js` 약 25줄, 서버 변경 불필요. "목표가 설정 → 도달 확인" 실제 투자 모니터링 워크플로우를 교실에서 체험.

- **거래 수량 퍼센트 프리셋 버튼 (25% / 50% / 100%)** (`static/js/app.js:1542-1553` `setMaxBuy()` / `setMaxSell()`, `static/index.html` 거래 모달): 현재 매수·매도 모달에 "최대" 버튼 1개만 있어, "보유 현금의 절반만 투자하고 싶다"처럼 중간 수량을 입력하려면 직접 숫자를 계산해야 함. `setMaxBuy()` → `setFracBuy(frac)` 로 시그니처 변경(`frac: 0.25 | 0.5 | 1.0`)하고, 거래 모달 "최대" 버튼 자리에 `[25%] [50%] [최대]` 버튼 3개로 교체. `setMaxSell()`도 동일하게 `setFracSell(frac)` 으로 변경. `app.js` 약 8줄 수정 + `index.html` 버튼 HTML 3줄. 분할 매수 교육 포인트를 버튼 하나로 자연 체험 가능하며, "왜 한꺼번에 매수하지 않았나요?" 토론 유도.

- **진행자 즉석 커스텀 퀴즈 출제 기능** (`education_data.py` QUIZ_QUESTIONS, `app.py:1575-1652` `get_quiz()` / `submit_quiz()`, 신규 `POST /api/rooms/<rid>/host/custom-quiz`): 현재 퀴즈가 `QUIZ_QUESTIONS`에 하드코딩된 문항만 출제되어, 교사가 수업 주제("오늘 배운 PER 개념을 퀴즈로 내겠습니다")에 맞는 커스텀 문제를 실시간으로 추가할 수 없음. `_custom_quiz: dict = {}  # rid -> deque(maxlen=5, [{id, q, a, ex}])` 전역 딕셔너리 추가. `POST /api/rooms/<rid>/host/custom-quiz {'question': str(2~200자), 'answer': bool, 'explanation': str(1~300자)}` 엔드포인트에서 검증 후 저장. `get_quiz()` 함수 앞부분에 `if _custom_quiz.get(rid): q = _custom_quiz[rid].popleft(); ...` 로 커스텀 문항을 기존 랜덤 문항보다 우선 출제. 진행자 퀴즈 설정 탭에 문제·정답(O/X)·해설 입력 폼. 서버 약 18줄 + 클라이언트 폼 약 12줄. 교사가 직전 설명 내용을 즉석 퀴즈로 출제해 이해도를 즉각 확인하는 형성평가 기능 실현.

- **포트폴리오 집중도 위험 경고 배너** (`static/js/app.js:1588-1698` `loadPortfolio()`, `GET /api/rooms/<rid>/portfolio` 응답 활용): 현재 포트폴리오 탭에 보유 종목 목록과 도넛 차트는 있지만, 단일 종목 과집중 여부를 명시적으로 경고하는 장치가 없어 "삼성전자에 전부 베팅"이 분산투자 실패임을 학생 스스로 인식하기 어려움. `loadPortfolio()` 응답 처리 직후에 `data.holdings.forEach(h => { if (h.current_value / data.total_value > 0.5) { /* 경고 배너 표시 */ } })` 추가: 단일 종목 비중 ≥50% 시 포트폴리오 탭 상단에 `⚠️ 고위험: [종목명]이 전체 자산의 XX%를 차지합니다 — 분산 투자를 고려하세요` 경고 카드 표시(배경 `var(--down)` 옅은 색). `app.js` 약 12줄, 서버 변경 불필요. "왜 1종목 집중이 위험한가요?" 투자 리스크 토론 자연 유발.

- **가상 경제 지표 패널 (기준금리·환율·코스피 시뮬레이션)** (`stock_service.py:127-141` `StockService.__init__()`, 신규 `GET /api/rooms/<rid>/indicators`, `static/js/app.js` 마켓 탭 상단): 현재 주가만 보이고 주식시장에 영향을 미치는 거시 경제 지표가 전혀 없어, "금리 인상이 주식시장에 미치는 영향" 같은 교육 컨텍스트를 연결하기 어려움. `StockService.__init__()` 에서 `self._indicators = {'rate': round(random.uniform(2.0, 5.0), 2), 'usd_krw': random.randint(1250, 1450), 'kospi': random.randint(2200, 2800)}` 초기화 후 가격 업데이트 주기(`_price_ttl`)마다 `±0.5%` 범위 미세 변동. `GET /api/rooms/<rid>/indicators` 신규 엔드포인트에서 `{'rate': float, 'usd_krw': int, 'kospi': int}` 반환. 마켓 탭 최상단에 "기준금리 X.XX% · 원달러 X,XXX원 · 코스피 X,XXX" 요약 배너 추가(30초 폴링). 서버 약 20줄 + 클라이언트 약 10줄. "금리가 오르면 왜 주식이 내리나요?" 교육 연결 고리 자연 형성.

- **종목 상세 모달 손익분기점 표시** (`static/js/app.js:1458-1488` `openStockModal()`, `GET /api/rooms/<rid>/portfolio` 응답 활용): 종목 모달에 현재가와 보유 수량은 표시되지만, 평균 매입단가(avg_price)와 손익분기점(현재 평단가)까지의 거리가 표시되지 않아 "팔면 얼마 남나?" 계산을 학생이 직접 해야 함. `openStockModal()` 에서 `port.holdings` 를 조회할 때 이미 `avg_price` 가 응답에 포함되므로(`app.py:1150` `'avg_price': h.avg_price` 반환), 해당 값을 `S.tradeAvgPrice = h?.avg_price || 0` 으로 캐시. 모달의 보유 수량 표시 아래에 `보유 시: 평단 ${krw(S.tradeAvgPrice)} · ${현재가 >= 평단 ? '▲'+pct(...) : '▼'+pct(...)} 수익` 한 줄 추가. `app.js` 약 6줄, 서버 변경 불필요. "지금 팔면 이득인지 손해인지"를 즉각 파악하는 실전 투자 UX 제공.

- **포트폴리오 섹터별 배분 요약 카드** (`static/js/app.js:1610-1634` `loadPortfolio()`, `GET /api/rooms/<rid>/portfolio` `holdings[].sector` 활용): 현재 포트폴리오 도넛 차트는 개별 종목명 + 현금 기준으로 분류되어, 섹터 분산 현황(IT에 얼마, 반도체에 얼마)을 한눈에 볼 수 없음. `holdings.reduce((acc, h) => { acc[h.sector] = (acc[h.sector] || 0) + h.current_value; return acc; }, {})` 로 섹터별 합산 후 포트폴리오 탭 도넛 차트 아래에 `섹터별: [반도체 XX%] [IT XX%] ...` 소형 pill 배지를 나열. 섹터 개수 ≥3 이면 "✅ 분산 투자 중" 초록 뱃지, 1개이면 "⚠️ 단일 섹터 집중" 경고 뱃지 추가. `app.js` 약 15줄, 서버 변경 불필요. "섹터 분산"이라는 추상 개념을 자신의 포트폴리오에서 직접 확인하며 체득.

### 제거/단순화할 것들

- **`host_news_interval()` `float()` 변환 try/except 없음 → 비정상 입력 시 500** (`app.py:997`, `app.py:999`): `svc.set_news_interval(float(d['news_seconds']))` 및 `svc.set_price_interval(float(d['price_seconds']))` 양 줄 모두 `d['news_seconds']`가 `"빠르게"` 같은 문자열이면 `float()` → `ValueError` → 처리되지 않은 예외 → 500. 같은 파일의 `host_adjust_time()`(`app.py:887`)은 `try: delta_minutes = float(...) except (TypeError, ValueError): return jsonify({'error': '숫자를 입력하세요.'}), 400` 패턴을 올바르게 적용 중. 해결: `app.py:996` 에 `try:` 블록 추가 후 `float(d['news_seconds'])` / `float(d['price_seconds'])` 를 묶고, `except (TypeError, ValueError): return jsonify({'error': '숫자를 입력하세요.'}), 400` 을 추가. 4줄 추가. `svc.set_news_interval()` / `svc.set_price_interval()` 내부에도 `max(5, min(300, float(seconds)))` 범위 검증이 있지만, `float()` 변환 자체가 먼저 실패하는 경우를 가드하지 않음.

- **`lottery_start()` `prize = float(...)` 및 `int(raw_round)` try/except 없음 → ValueError 500** (`app.py:1418`, `app.py:1431`): `prize = float(d.get('prize', 0))` — `d.get('prize')` = `"천만"` 시 `ValueError` → 500. 이어서 `round_n = int(raw_round)` (`app.py:1431`)도 `raw_round` = `"두 번째"` 시 동일 문제. 동일 파일의 `create_room()`(`app.py:708`)은 이미 `int(d.get('duration_minutes', 30))` 를 try/except 없이 사용해 2026-09-11에 별도 지적됐으나 수정되지 않았고, `lottery_start()`는 아직 별도 지적이 없음. 해결: `app.py:1417` 이후에 `try: prize = float(d.get('prize', 0)) \nexcept (TypeError, ValueError): return jsonify({'error': '상금은 숫자여야 합니다.'}), 400`; `app.py:1430-1431` `int(raw_round)` 도 `try/except (TypeError, ValueError)` 로 감싸기. 총 6줄 추가로 malformed JSON 입력에 의한 500 완전 차단.

- **`quiz_settings()` POST float 변환 try/except 없음 → ValueError 500** (`app.py:1760-1761`): ```_quiz_settings[rid] = {'reward_pct': max(0, min(10, float(d.get('reward_pct', 1.0)))), 'penalty_pct': max(0, min(10, float(d.get('penalty_pct', 0.5))))}``` — `d.get('reward_pct')` = `"많이"` 시 `float()` → `ValueError` → 500. `submit_quiz()` 가 이 딕셔너리를 읽는 `_quiz_settings.get(rid, {})` 는 안전하지만, 진행자가 퀴즈 설정을 잘못 입력하면 설정 엔드포인트 자체가 터짐. 해결: `app.py:1758` (POST 분기 진입 직후)에 `try: rp = max(0, min(10, float(d.get('reward_pct', 1.0)))); pp = max(0, min(10, float(d.get('penalty_pct', 0.5)))) \nexcept (TypeError, ValueError): return jsonify({'error': '비율은 0~10 사이 숫자여야 합니다.'}), 400` 로 교체. 3줄 수정.

- **`withdraw_deposit()` `RoomMember` None 체크 없음 → AttributeError 500** (`app.py:1275-1278`): 예금 해지 처리에서 `m = RoomMember.query.filter_by(room_id=rid, user_id=user.id).first()` 후 즉시 `db.session.refresh(m)` 호출. DB 무결성 오류나 데이터 불일치로 `m = None` 이 되면 `db.session.refresh(None)` → `TypeError: argument of type 'NoneType' is not iterable` 또는 `AttributeError` → 500. `m.cash += dep.amount`도 동일. 같은 파일 `create_deposit()`(`app.py:1246`)은 `if m.cash < amount` 전에 이미 m 존재 여부를 앞선 `if not m: return 403`로 보호하는 패턴과 불일치. 해결: `app.py:1275` `m = RoomMember...` 다음 줄에 `if not m: db.session.rollback(); return jsonify({'error': '참여자 정보를 찾을 수 없습니다.'}), 404` 1줄 추가. 예금 UPDATE가 이미 커밋된 상태이므로 `db.session.rollback()` 대신 원금 환불 로직이 필요할 수 있어 이 순서로 처리하는 것이 중요.

- **`get_history()` x축 레이블이 항상 일별 날짜 포맷(`%Y-%m-%d`) — 기간에 무관하게 동일 날짜 형식 사용** (`stock_service.py:319`): `date_str = datetime.utcfromtimestamp(now - i * 86400).strftime('%Y-%m-%d')` — `period='1d'`(당일 5분봉, `n_bars=30`) 이든 `period='3mo'`(90일봉) 이든 x축 레이블이 항상 `YYYY-MM-DD` 일별 날짜로 생성됨. `interval` 파라미터(`'5m'`, `'30m'`, `'1d'`, `'1wk'`)는 `get_history()` 시그니처에는 있지만 본문에서 전혀 사용되지 않음. `period='1d'`(오늘) 30개 봉의 날짜가 모두 동일 날짜(`today - 0 * 86400 = today`)로 찍혀 차트 x축이 같은 날짜 30개로 표시되는 시각적 혼란 발생. 해결: `stock_service.py:316-319` 날짜 생성 부분을 `interval_secs = {'5m': 300, '30m': 1800, '1d': 86400, '1wk': 86400 * 7}.get(interval, 86400)` → `now - i * interval_secs` 로 타임스탬프 계산하고, `fmt = '%H:%M' if interval in ('5m','30m') else '%m/%d' if interval == '1d' else '%Y-%m-%d'` 으로 레이블 포맷 분기 적용. 5줄 수정으로 기간별 차트 x축 레이블이 의미 있는 시간·일자·주차로 표시됨.

- **`export_rankings()` 학번-이름 분리 로직이 공백 기준 `split(' ', 1)` — 이름에 공백 포함 시 학번에 이름 일부가 들어가는 오작동** (`app.py:1784-1788`): ```parts = e['username'].split(' ', 1); sid = parts[0] if len(parts) > 1 else ''; name = parts[1] if len(parts) > 1 else e['username']``` — 학생이 학번 없이 `"홍 길동"` 같이 공백이 포함된 이름을 입력하면 `sid = '홍'`, `name = '길동'` 으로 잘못 분리되어 엑셀에 학번 칸에 이름 일부가 들어감. `doAuth()` (`app.js:78`) 의 `u = ${sid} ${name}` 에서 sid가 숫자인지 보장하지 않음. 해결: `app.py:1784` 분리 로직을 `import re; m = re.match(r'^(\d{1,10})\s+(.+)$', e['username']); sid = m.group(1) if m else ''; name = m.group(2) if m else e['username']` 로 교체 — 첫 토큰이 숫자인 경우만 학번으로 해석. 2줄 수정으로 이름에 공백이 포함된 학생의 엑셀 출력 오류 해소. 클라이언트 입력 폼의 학번 필드(`doAuth()` 호출부)에도 `\d+` 정규식 검증 추가 권장.
...
- **차트 위 내 거래 시점 오버레이 표시** (`app.py:1067-1076` `get_chart()`, `app.py:1178-1198` `get_transactions()`, `static/js/app.js` 차트 렌더 함수): 개별 종목 차트에 랜덤 생성 가격 히스토리만 표시되고, 내가 언제 해당 종목을 매수/매도했는지 전혀 표시되지 않아 "내 타이밍이 좋았나?" 자기 평가 불가. `get_transactions()` 응답에서 현재 차트 심볼과 일치하는 거래의 `timestamp`를 뽑아 `get_chart()` 응답의 `history[].date` 배열과 대조, 가장 가까운 봉 인덱스에 ▲(매수, 초록) / ▼(매도, 빨강) 마커를 Canvas 직접 드로잉(`ctx.fillText('▲', x, y)`). 서버 변경 불필요, `app.js` 차트 렌더링 로직 약 25줄 추가. "내가 고점에서 샀구나", "뉴스 직후 팔았어야 했는데" 자기 성찰 교육 포인트 자연 발생.

- **게임 시작 전 리서치 페이즈 (N분 거래 금지 탐색 시간)** (`app.py:808-821` `start_room()`, `app.py:1081-1088` `trade()`, `app.py:549-580` `room_dict()`): 게임 시작 즉시 거래가 열려 학생들이 충분한 분석 없이 "삼성전자 전부 매수" 같은 즉흥 매매를 함. `start_room()` POST 파라미터에 `research_minutes: int (0~10)` 옵션을 추가하고, 설정 시 `room.research_until = now + timedelta(minutes=research_minutes)` (신규 컬럼)를 저장. `trade()` 핸들러 진입부에 `if room.research_until and datetime.utcnow() < room.research_until: return jsonify({'error': f'리서치 타임 중 — {int((room.research_until - datetime.utcnow()).total_seconds())}초 후 거래 시작'}), 400` 삽입. `room_dict()`에 `'research_seconds_left': max(0, int(...))` 포함해 클라이언트 "거래 시작까지 02:30" 카운트다운 표시. 서버 약 12줄 + 클라이언트 카운트다운 약 8줄. "정보 없이 투자 vs 조사 후 투자" 실험으로 투자 전 분석의 중요성 체험.

- **진행자 종목별 거래 금지/허용 토글 (거래 제한 목록)** (`app.py:1081-1101` `trade()`, 신규 `_banned_symbols` 전역 dict, `POST /api/rooms/<rid>/host/ban-symbol`): 진행자가 특정 종목(예: 삼성전자)을 거래 금지해 "삼성전자 없이 시장이 어떻게 변하나?" 의존도 교육 시나리오나 "이번 게임은 IT 섹터만 거래" 집중 실험을 연출할 수단이 없음. `_banned_symbols: dict = {}  # rid -> set(str)` 전역 딕셔너리 추가. `POST /api/rooms/<rid>/host/ban-symbol {'symbol': str, 'banned': bool}` 엔드포인트에서 집합에 추가/제거. `trade()` 핸들러 `symbol not in STOCKS` 검사 직후 `if symbol in _banned_symbols.get(rid, set()): return jsonify({'error': f'{STOCKS[symbol]["name"]}은(는) 거래 제한 종목입니다.'}), 403` 삽입. `get_stocks()` 응답 항목에 `'is_banned': bool` 추가해 클라이언트가 종목 행에 🚫 아이콘 표시 및 매수/매도 버튼 비활성화. `_end_room()`에서 `_banned_symbols.pop(rid, None)` 정리. 서버 약 15줄 + 클라이언트 약 8줄.

- **팀 모드: 학생 팀 배정 및 팀 평균 수익률 순위표** (`app.py` 신규 `_team_assignments: dict`, `POST /api/rooms/<rid>/host/teams`, `GET /api/rooms/<rid>/rankings?view=teams`): 개인 순위만 있어 팀 협업·팀 전략 비교 수업이 불가능. `_team_assignments: dict = {}  # rid -> {uid: team_name}` 전역 딕셔너리 추가. `POST /api/rooms/<rid>/host/teams {'assignments': {str(uid): str(team_name)}}` 로 일괄 배정 (진행자 전용). `GET /api/rooms/<rid>/rankings?view=teams` 파라미터 수신 시 기존 `_compute_leaderboard()` 결과를 `team_name`으로 groupby → 팀별 평균 `gain_pct`·평균 `total_value`·팀원 수 집계 후 팀 순위표 반환. `room_dict()`에 `'team_mode': rid in _team_assignments` 포함. 클라이언트 순위표 탭에 "개인 순위 | 팀 순위" 토글. 서버 약 25줄 + 클라이언트 약 15줄. "팀 수익률이 개인 전략에 어떻게 의존하나?" 협업·분산투자 교육 시나리오 가능.

- **복권 당첨 번호별 학생 선택 빈도 히트맵** (`app.py:464-504` `_do_reveal()`, `app.py:1446-1479` `get_lottery()`, `static/js/app.js` 복권 탭): 복권 결과 화면에서 당첨 번호만 강조될 뿐, 학생들이 어떤 번호를 얼마나 선택했는지 집계가 없어 "사람들은 왜 같은 번호를 고를까?" 교육 토론이 빈약. `_do_reveal()` 내 루프 완료 직후에 `from collections import Counter; cur['freq'] = dict(Counter(n for picks in cur['picks'].values() for n in picks))` 1줄 추가. `get_lottery()` `state=revealed` 분기에 `'freq': cur.get('freq', {})` 포함. 클라이언트 1~45 번호 그리드에서 각 번호의 `freq[n] / total_participants` 비율을 `opacity`로 표현 (인기 번호일수록 진한 색). 서버 3줄 + 클라이언트 그리드 렌더 약 10줄. "무작위 선택이 진짜 균등한가? 왜 1, 7, 14가 인기인가?" 행동경제학 교육 자료.

- **게임 중 학생 개인 투자 일지 메모 기능** (`static/js/app.js` 포트폴리오 탭, `localStorage`): 학생이 "삼성전자 상승 예상 — 반도체 뉴스" 같은 투자 판단 근거를 실시간 기록하는 메모 공간이 없어, 게임 후 "왜 그 결정을 했나?" 디브리핑이 기억에 의존. 포트폴리오 탭 하단에 `<textarea maxlength="500" placeholder="📝 투자 판단 메모...">` 추가, 입력 500ms 후 `localStorage.setItem('memo_' + rid, text)` 디바운스 저장. 게임 종료 후 결과 화면에서 "내 투자 일지"로 상단 카드에 표시. 서버 변경 불필요, `app.js` 약 12줄 + `index.html` 1줄. 교사가 "메모를 보며 그 판단의 이유를 발표해봐요" 수업으로 메타인지 학습 사이클 완성.

### 제거/단순화할 것들

- **`minigame_spin()` 스핀 횟수 제한 레이스 컨디션 — 동시 요청 시 3회 초과 가능** (`app.py:1372-1405`): `spins_used = RoomTransaction.query.filter_by(room_id=rid, user_id=user.id, action='RLT').count()` → `if spins_used >= 3: return` 체크와 실제 RLT 거래 INSERT가 `_get_member_lock(rid, user.id)` 밖에서 실행됨. 같은 학생의 spin 요청 2개가 동시 도착하면 둘 다 `spins_used=2` 를 읽어 검증 통과 후 모두 실행 → 4회 spin 가능. `trade()`, `create_deposit()`는 lock 내에서 `db.session.refresh()` 후 상태 재확인하는 패턴을 일관되게 사용하지만 `minigame_spin()`은 예외. 해결: `app.py:1376` 아래 bet 파싱 이후의 전체 spin 실행 블록 (`shortfall` 계산 ~ `db.session.commit()`)을 `with _get_member_lock(rid, user.id):` 로 감싸고, 블록 내부 첫 줄에 `spins_used = RoomTransaction.query.filter_by(room_id=rid, user_id=user.id, action='RLT').count(); if spins_used >= 3: return jsonify({'error': '기회를 모두 사용했습니다.'}), 400` 재확인 추가. 약 5줄 수정.

- **`enter()` 동시 닉네임 중복 생성 시 `IntegrityError` 미처리 → 500** (`app.py:606-618`): `user = User.query.filter_by(username=u).first()` 후 `user = User(username=u); db.session.add(user); db.session.commit()` 사이 레이스 컨디션. 동일 닉네임으로 두 요청이 동시 도착 시 둘 다 `user = None` 수신 → 둘 다 INSERT 시도 → 두 번째 `db.session.commit()` 에서 `IntegrityError` (UNIQUE 제약 위반) → 처리되지 않은 예외 → 500 반환. `join_room()` (`app.py:728-730`)은 이미 `try/except IntegrityError:` + `db.session.rollback()` 패턴을 올바르게 적용. 해결: `app.py:612` `db.session.commit()` 블록을 `try: db.session.commit() \nexcept IntegrityError: db.session.rollback(); user = User.query.filter_by(username=u).first()` 로 감싸기. 4줄 추가로 동시 접속 학생 충돌 500 오류 해소.

- **`SECRET_KEY` 기본값 공개 코드 노출 → 세션 쿠키 위조 가능 보안 취약점** (`app.py:24`): `app.secret_key = os.environ.get('SECRET_KEY', 'mock-stock-game-secret-2024')` — 환경 변수 미설정 시 기본값이 GitHub에 공개되어 있어 공격자가 임의 `user_id`를 포함한 서명된 Flask 세션 쿠키를 직접 생성 가능. `user_id=1` (진행자 계정일 가능성 높음)로 위조하면 모든 `@login_required` 엔드포인트 무단 접근 및 `host_adjust()`, `host_force_price()`, `end_room()` 같은 진행자 전용 기능 악용 가능. 해결: `app.py:25` 이후에 `if not os.environ.get('SECRET_KEY'): app.logger.warning('[security] SECRET_KEY 미설정 — 기본값 사용 중. 프로덕션 배포 시 반드시 환경 변수를 설정하세요!')` 경고 로그 추가(1줄). 장기적으로는 `if not app.debug and not os.environ.get('SECRET_KEY'): raise RuntimeError('SECRET_KEY must be set in production')` 으로 배포 자체를 차단.

- **`host_market_event()` bare `except` 절 — `KeyboardInterrupt`/`SystemExit` 까지 포착** (`app.py:1672`): `try: pct = float(d.get('pct', 0)) \nexcept: return jsonify({'error': '잘못된 변동률'}), 400` — Python bare `except`는 `BaseException` 전체를 포착하므로 `KeyboardInterrupt`, `SystemExit`, `GeneratorExit`까지 삼킴. gunicorn이 `SIGTERM` 으로 worker를 종료할 때 이 핸들러 실행 중이면 종료 신호 처리가 방해될 수 있음. 동일 파일의 `host_adjust_time()` (`app.py:887`), `host_adjust()` (`app.py:949`)는 올바르게 `except (TypeError, ValueError):`를 사용. 해결: `app.py:1672` `except:` → `except (TypeError, ValueError):` 교체(1줄). 동일 bare `except` 패턴이 `trade()` (`app.py:1095`) `except: return jsonify({'error': '수량 오류'})` 에도 존재 — 함께 수정 권장.

- **`models.py` FK 컬럼 DB 인덱스 누락 — PostgreSQL 배포 시 전체 테이블 스캔** (`models.py:47-65`): `RoomMember.room_id`, `RoomHolding.room_id`/`user_id`, `RoomTransaction.room_id`/`user_id`, `Deposit.room_id`/`user_id` 등 `filter_by()`에 가장 자주 쓰이는 FK 컬럼에 SQLAlchemy `db.Index` 선언이 없음. SQLAlchemy는 PK와 UniqueConstraint 제외 FK 컬럼에 인덱스를 자동 생성하지 않음. 학생 30명 × 거래 100건 = 3,000 행 `room_transactions`에서 `filter_by(room_id=rid, user_id=uid)` 시 PostgreSQL Seq Scan 발생 — `get_transactions()`, `_compute_leaderboard()`, `_do_reveal()` 등 다수 핵심 경로에 영향. 해결: `models.py` 클래스 정의 아래에 `db.Index('ix_rm_room', RoomMember.room_id)`, `db.Index('ix_rt_room_user', RoomTransaction.room_id, RoomTransaction.user_id)`, `db.Index('ix_rh_room_user', RoomHolding.room_id, RoomHolding.user_id)`, `db.Index('ix_dep_room_user', Deposit.room_id, Deposit.user_id)` 추가. `db.create_all()` 이 자동 생성하므로 기존 앱 재시작으로 적용. 4줄 추가로 PostgreSQL 쿼리 성능 수십 배 개선 가능.

- **`Room.query.get_or_404()` 등 SQLAlchemy 1.x 레거시 Query API와 2.0 스타일 혼용** (`app.py:760`, `811`, `823`, `836`, `855`, `879`, `908`, `917` 등 20+ 곳): 코드베이스가 SQLAlchemy 2.0 스타일 `db.session.get(Model, pk)` (올바른 사용 — `app.py:153`, `243` 등)와 1.x 레거시 `Model.query.get_or_404(pk)`, `Model.query.filter_by(...).first()` 패턴을 동일 파일에서 혼용. Flask-SQLAlchemy 3.0+에서 `Model.query`는 deprecated되고 `db.session.execute(select(Model))` 권장. 레거시 API는 향후 메이저 버전에서 제거 예정 — 혼용 상태에서 Flask-SQLAlchemy 업그레이드 시 일부 경로만 갑자기 동작 불능 위험. 해결: `Room.query.get_or_404(rid)` → `db.get_or_404(Room, rid)` (Flask-SQLAlchemy 3.0 신규 API)로 점진적 교체. 우선순위 높은 hot-path인 `get_room()` (`app.py:760`), `start_room()` (`app.py:811`), `trade()` (`app.py:1083`) 순서로 수정 시작. 파일 전체 약 20곳 교체 필요.
...
- **호스트 대시보드에 학생별 현재 보유 종목 조회 패널 추가** (`app.py:700-721`, `host_member_transactions()`): 진행자는 현재 거래 내역(`/host/members/<uid>/transactions`) 조회는 가능하지만 학생이 지금 어떤 주식을 몇 주 보유 중인지 실시간으로 볼 방법이 없음. `GET /api/rooms/<rid>/host/members/<uid>/holdings` 엔드포인트를 신규 추가해 `RoomHolding.query.filter_by(room_id=rid, user_id=uid).all()` 결과와 현재가(`svc.get_price()`)를 묶어 반환. 기존 `get_portfolio()` (`app.py:868-899`) 로직을 거의 그대로 재사용하되 `@login_required` + 호스트 권한 검증만 추가하면 약 20줄. 호스트 순위판의 학생 이름 행 클릭 시 거래 내역 모달(`host/members/<uid>/transactions`) 옆에 "현재 포트폴리오 보기" 탭을 추가하면 교사가 특정 학생의 투자 전략을 수업 중에 바로 띄워 설명하는 시나리오가 가능해짐.

- **`ending_soon` 플래그 활성화 시 참가자 전체화면 카운트다운 알림 구현** (`app.py:405`, `app.py:628-636`, `app.js` 폴링 루프): `room_dict()`에서 `'ending_soon': room.id in _ending_soon` (`app.py:405`)을 이미 반환하고, `end_room()` (`app.py:628-636`)에서 잔여 60초 초과 시 `end_time`을 60초 뒤로 당겨 `_ending_soon`에 추가함. 그러나 app.js 폴링에서 `S.room.ending_soon` 값이 바뀌어도 학생 화면에는 아무 특별 알림이 없어 종료가 임박했음을 체감하기 어려움. 폴링 루프(`refreshRoomStatus()` 또는 주기 콜백)에서 `if (room.ending_soon && !S._endingSoonShown) { toast('⏰ 1분 후 게임이 종료됩니다!', 'error'); S._endingSoonShown = true; }` 약 4줄 추가만으로 해결. 타이머 색상도 붉게 바뀌도록 CSS 클래스 토글 추가하면 더욱 효과적.

- **실제 인게임 가격 변화 기록 및 차트 반영** (`stock_service.py:196-212`, `stock_service.py:303-332`, `get_price()`): 현재 `get_history()` (`stock_service.py:303`)는 현재 종가에서 역산한 **랜덤 OHLC** 를 반환하므로 게임 중 실제로 발생한 가격 이동과 전혀 무관. 예를 들어 진행자가 `force_price()`로 삼성전자를 +30% 올려도 학생의 차트에는 그 이벤트가 나타나지 않음. `StockService.__init__`에 `self._tick_log: dict = {}` (sym → list of (ts, price)) 를 추가하고, `get_price()` 내 가격 갱신 시(`app.py:207-211` 로직에 대응하는 `stock_service.py:206-212`) `self._tick_log.setdefault(sym, []).append((now, new_price))` 로 기록. `get_history()` 는 `_tick_log`가 있으면 이를 우선 사용해 OHLC 변환(`bars` 구성)하도록 교체. 메모리 부담은 방당 종목 60개 × 최대 300틱 × 2개 float ≈ 수십 KB 수준으로 무시 가능.

- **호스트의 게임 중 전체 참가자 공지 메시지 전송** (신규 엔드포인트, `app.py` 전반): 현재 호스트가 학생들에게 게임 중 임의 텍스트를 보낼 방법이 없음(뉴스 트리거, 가격 조정은 있지만 순수 텍스트 공지 없음). `POST /api/rooms/<rid>/host/announcement` 엔드포인트를 추가해 `{'message': '5분 남았습니다. 포트폴리오를 정리하세요!'}` 형태의 메시지를 `_announcements[rid] = {'message': msg, 'ts': time.time()}` in-memory로 저장. 참가자의 뉴스 폴링 응답(`get_room_news()`, `app.py:797-801`) 에 `'announcement'` 필드를 함께 내려주거나, 별도 `GET /api/rooms/<rid>/announcement` 를 폴링. 프런트는 `toast()` 또는 폭탄뉴스 팝업(`bomb-news-popup`, `index.html:843-855`)과 유사한 별도 색깔 배너로 표시. 실제 수업에서 가장 자주 요청받는 기능으로 교사 UX 만족도에 직결.

- **게임 종료 후 결과 화면에 교육적 거래 통계 추가** (`app.py:1474-1542`, `export_rankings()`, `index.html:616-657`): `export_rankings()`는 순위·수익률만 엑셀에 담고, 결과 화면도 순위표·자산 바 차트만 표시함. `RoomTransaction` 테이블에는 전체 거래 데이터가 있으므로, `_compute_leaderboard()` 호출 시점에 추가 집계(`db.session.query(RoomTransaction.symbol, func.count()).filter_by(room_id=rid, action='BUY').group_by(RoomTransaction.symbol).order_by(func.count().desc()).limit(5).all()`)로 "가장 많이 거래된 종목 Top 5", "평균 거래 횟수", "퀴즈 정답률" 등을 계산해 결과 API 응답에 포함. 결과 화면 하단에 통계 섹션 추가로 수업 마무리 토론 자료 자동 생성.

### 제거/단순화할 것들

- **`_quiz_state`·`_quiz_settings`·`_quiz_history`·`_rlt_active`·`_roulette_config` 가 서버 재시작(Render 슬립) 시 완전 손실** (`app.py:267-268`, `app.py:351-354`, `app.py:1314-1316`): Render 무료 플랜은 15분 비활성 후 dyno를 슬립시킴. 재시작 후 복구 코드는 `_lots`(복권) 의 `done` 집합만 DB에서 읽어 복원하지만(`app.py:276-280`) 퀴즈 상태(`_quiz_state`), 퀴즈 설정(`_quiz_settings`, 진행자가 게임 중 변경한 보상/패널티 비율), 퀴즈 기록(`_quiz_history`), 룰렛 설정(`_roulette_config`), 활성 룰렛 카운터(`_rlt_active`)는 전부 소실됨. 30분 수업 중 15분 이상 쉬거나 학생들이 조용히 앉아 있으면 복권 및 룰렛 진행 상태가 증발할 수 있음. 단기 해결책: `_quiz_settings`·`_roulette_config` 를 `Room` 테이블에 JSON 컬럼으로 저장해 재시작 후 복원; 장기 해결책: Render의 "keep-alive" 크론 또는 외부 헬스체크 서비스 도입.

- **`get_prev_close()` 가 게임 시작 이후 한 번도 갱신되지 않아 변동률 의미 왜곡** (`stock_service.py:149`, `stock_service.py:300-301`, `app.py:754-761`): `_prev[sym]` 은 `_init_prices()` (`stock_service.py:149`) 에서 게임 시작 시점 가격으로만 설정되고 이후 `get_price()` 에서 가격이 갱신되어도 `_prev`는 절대 바뀌지 않음. 따라서 `get_stocks()` (`app.py:754-761`) 의 `change_pct = (price - prev) / prev * 100` 은 "직전 틱 대비 변동"이 아니라 "게임 시작 시 대비 누적 변동"을 의미함. UI 레이블이 `▲/▼ %` 로 주식 앱 관례의 당일 변동처럼 보여 학생들을 혼동시킴. `get_price()` 에서 가격 갱신 시 직전 가격을 `_prev[sym]`에 저장(`self._prev[symbol] = price` — 갱신 전 값)하면 의미가 명확해지고, 틱 간 변동률을 보여줄 수 있어 교육적으로도 훨씬 자연스러움.

- **QR 코드 `colorDark`·`colorLight` 반전으로 일부 QR 리더 인식 실패** (`static/js/app.js:202-203`): `new QRCode(el, { colorDark: '#e6edf3', colorLight: '#0d1117', ... })` 에서 `colorDark`(다크 모듈 색)에 밝은 색(`#e6edf3`)이, `colorLight`(배경 색)에 어두운 색(`#0d1117`)이 지정되어 있어 결과물이 "밝은 점, 어두운 배경" 구조의 반전 QR임. QR 명세(ISO/IEC 18004)는 어두운 모듈·밝은 배경을 규정하며, 특히 구형 Android 카메라 앱·키오스크·Windows 카메라 앱 등에서 반전 QR 인식에 실패하는 사례가 빈번. 교실 환경에서 학생의 스마트폰 기종이 다양하므로 실제 수업 진행 장애로 이어질 수 있음. `colorDark: '#000000', colorLight: '#ffffff'` 로 변경하면 다크 테마 배경 위에서도 대부분의 QR 리더가 인식하며(QR 라이브러리가 캔버스에 독립적으로 렌더링), `_makeQR()` 함수 호출부에 흰 배경 컨테이너 스타일(`background:#fff;padding:6px`)을 추가하면 미관도 유지됨.

- **`kick_member()` 가 `waiting` 상태 전용으로 제한되어 게임 중 이탈 학생 처리 불가** (`app.py:657`): `if room.status != 'waiting': return jsonify({'error': '대기 중인 방에서만 강퇴할 수 있습니다.'}), 400` 로 인해 게임 시작 후에는 교사가 돌발 상황(학생 비정상 행동, 잘못 입장한 학생)에 대응할 수 없음. 게임 중 강퇴는 보유 주식·예금 청산 후 현금 반환이 필요해 복잡하지만, 최소한 `waiting → active` 로 상태 조건을 완화하고, 강퇴 시 `_liquidate_shortfall()` 대신 단순히 `RoomMember`를 `status='kicked'` 컬럼(신규)으로 마킹해 거래 차단·순위 제외·보유 종목 동결 처리하는 경량 구현이 현실적. `app.py:868` `get_portfolio()` 와 `app.py:818` `trade()` 의 멤버 조회를 `status='active'` 필터 조건만 추가하면 됨.

- **`_next_price()` 가격 상하한 `base * 0.6 ~ base * 1.4` 고정으로 장시간 게임 시 모든 종목이 경계에 수렴** (`stock_service.py:161`): 가격이 `base * 1.4` 에 도달하면 `drift > 0` 이어도 라운딩 결과가 `1.4 * base` 그대로 반환되어 연속으로 상한에 붙는 "동결" 현상 발생. 변동성(`vol`)이 높은 종목(TSLA `0.055`, HMM `0.050`, CLTRI `0.045`)은 60분 이상 게임에서 경계에 도달할 확률이 높음. 두 가지 개선안: ① 상하한을 `base * 0.4 ~ base * 2.0` 으로 확장해 더 극적인 가격 움직임 허용 ② 게임 진행 시간 비율(`elapsed/total`)에 비례해 상하한을 동적으로 조절(`max_mult = 1.4 + 0.6 * elapsed_ratio`). 진행자의 `force_price()`는 이미 `base * 0.3 ~ base * 3.0` 범위(`stock_service.py:247`)를 허용해 자연 변동과 강제 조정 간 일관성도 없음.

## 2026-08-13 (2차)

### 추가하면 좋을 기능

- **룰렛 베팅 빠른 선택 버튼 (10%/25%/50%/전액) 추가** (`static/js/app.js` 룰렛 베팅 입력부, 현재 `setRltPct()` 참고): 예금 입력부에는 `setDepPct(pct)` 를 통한 25%/50%/75%/전액 빠른 선택 버튼이 이미 구현되어 있고(`setDepPct()` 인근), 룰렛 베팅도 동일한 `setRltPct(pct)` 함수가 존재하지만 해당 함수를 호출하는 UI 버튼이 빠져 있음. 참가자가 룰렛 베팅 금액을 직접 타이핑해야 해서 모바일 환경에서 특히 불편하고 수업 중 시간이 낭비됨. 예금 UI의 `<button onclick="setDepPct(0.25)">25%</button>` 패턴을 그대로 복사해 룰렛 베팅 입력 바로 위에 `<button onclick="setRltPct(0.1)">10%</button>` 등 4개 버튼을 `index.html` 룰렛 섹션에 추가하고, `setRltPct(pct)` 가 `S.myAssets.total_assets * pct` 를 베팅 입력란에 채우도록 연결. 3줄 HTML + 1줄 JS 수정으로 해결.

- **주식 상세 모달 포트폴리오 API 캐싱으로 중복 요청 감소** (`static/js/app.js:1372`): `openStockModal()` 은 종목 카드를 탭할 때마다 `GET /api/rooms/${rid}/portfolio` 를 호출해 보유 현금·주식 수를 갱신함. 학생이 여러 종목을 빠르게 열어보면 매번 DB 조회가 발생하며, Render 무료 플랜의 슬로우 DB 환경에서 체감 지연이 생길 수 있음. `S.portfolioCache = { data: null, ts: 0 }` 를 추가하고, `openStockModal()` 에서 `Date.now() - S.portfolioCache.ts < 5000` 이면 캐시를 재사용하도록 수정. 매매 성공 후에는 `S.portfolioCache.ts = 0` 으로 무효화. 5초 캐시만으로도 연속 종목 탐색 시 API 호출 80% 이상 감소. `openStockModal()` 및 매매 성공 콜백 각 ~4줄 수정.

- **호스트 마켓 그리드에 가격 변동 플래시 애니메이션 추가** (`static/js/app.js:316-360`, `loadHostMarket()`): 참가자 뷰의 종목 그리드는 `flash-up`/`flash-down` CSS 클래스를 추가해 가격 변동을 시각적으로 강조하는데(`app.js:1341-1351` 인근), 호스트 마켓 탭(`loadHostMarket()`)은 이전 가격(`S._prevHostPrices`)을 기록하지 않아 가격이 올라도 아무 시각 피드백이 없음. `loadHostMarket()` 갱신 루프에서 이전 가격 딕셔너리를 관리하고, 참가자 뷰와 동일한 `flash-up`/`flash-down` 클래스 토글을 적용. 호스트가 `force_price()` 또는 섹터 이벤트 후 즉각적인 가격 변화를 시각으로 확인하는 데 유용하며, 기존 CSS 클래스·애니메이션을 그대로 재사용하므로 추가 스타일링 불필요. 약 10줄 JS 추가.

- **게임 시작 확인 모달에 현재 접속 참가자 수 표시** (`static/js/app.js:248-257`, `doStartGame()`): `doStartGame()` 은 `confirm('게임을 시작하시겠습니까?')` 브라우저 기본 다이얼로그만 표시하고, 현재 로비에 몇 명이 있는지를 알려주지 않음. 교사가 "다 들어왔나요?" 구두 확인 없이 실수로 시작해버리는 상황이 생김. 커스텀 확인 모달로 교체하거나, 기존 `confirm()` 문자열을 `confirm(\`현재 ${S.lobbyMembers.length}명이 대기 중입니다. 게임을 시작하시겠습니까?\`)` 로 바꾸는 것만으로도 해결(1줄 수정). `S.lobbyMembers` 배열은 `loadPLobbyMembers()` 또는 `loadHostLobbyMembers()` 에서 이미 관리되므로 별도 API 호출 불필요.

- **O/X 퀴즈 결과 화면에 정답률 히스토그램 추가** (`app.py:1280-1313`, `quiz_end()`, `index.html` 퀴즈 섹션): `quiz_end()` 는 퀴즈 종료 시 `_quiz_history[rid]` 에 `{question, correct_answer, results: [{user_id, answer, correct, delta}...]}` 형태로 결과를 기록함. 하지만 현재 호스트 화면에는 참가자별 O/X 아이콘 표시만 있고, "이 문제 전체 정답률은 몇 %?" 를 볼 방법이 없음. 퀴즈 종료 응답에 `correct_count / total_count` 를 추가해(`app.py:1313` 응답 dict에 `'correct_rate'` 필드 추가) 호스트 화면에 "✅ 정답률 67%" 배너를 1줄로 표시. 이후 `_quiz_history` 를 순회해 문제별 정답률 바 차트를 그리면 교사가 어느 개념을 학생들이 어려워하는지 수업 중 즉석 파악 가능. 백엔드 3줄 + 프런트 5줄 수정.

### 제거/단순화할 것들

- **`lobby_members()` N+1 쿼리 — 루프 내 개별 User 조회** (`app.py:672-674`): `for m in RoomMember.query.filter_by(room_id=rid).all()` 루프 내에서 `db.session.get(User, m.user_id)` 를 매 반복마다 호출함. 참가자가 30명이면 31번의 DB 쿼리가 순차 발생. 로비 화면은 5초마다 이 API를 폴링하므로, 30명 방에서 분당 최소 6회 × 31쿼리 = 186번의 쿼리가 발생. 해결: 먼저 멤버 ID 목록을 수집한 뒤 `User.query.filter(User.id.in_(member_ids)).all()` 한 번으로 일괄 조회하고 딕셔너리로 변환. `app.py:671-674` 를 3줄에서 2줄 쿼리로 교체하면 N+1 → 2쿼리로 감소. SQLAlchemy `joinedload` 를 `RoomMember` 관계에 추가하는 방법도 있으나 모델 변경이 필요해 in_() 방식이 더 간단.

- **`get_rankings()` 미멤버 접근 차단 미비로 전체 순위 노출** (`app.py:904-912`): `GET /api/rooms/<rid>/rankings` 는 `@login_required` 만 있고 멤버십 검증이 없어, 로그인한 사용자라면 누구나 모든 방의 순위표(학생 이름, 자산 규모 등)를 조회할 수 있음. 학교 환경에서 다른 반 학생이 URL만 알면 타 반 학생들의 자산 순위를 볼 수 있는 개인정보 문제. `app.py:907-908` 에 `if room.host_id != user.id and not RoomMember.query.filter_by(room_id=rid, user_id=user.id).first(): return jsonify({'error': '권한 없음'}), 403` 3줄을 추가하면 해결. 같은 패턴이 `kick_member()`, `host_adjust()` 등에는 이미 적용되어 있어 코드 일관성 측면에서도 필요.

- **`join_room()` 일시정지(`paused`) 상태 방 입장 허용으로 게임 중 신규 합류 가능** (`app.py:499`): `room.status == 'ended'` 만 차단하고 `paused` 상태는 허용함. 룰렛·복권 등 미니게임 진행 중 방이 일시정지된 틈에 코드를 아는 학생이 새로 입장하면 `RoomMember` 가 생성되어 잔액(`starting_cash`)이 주어지고 순위에 포함되어버림. 진행자가 의도하지 않은 시점에 참가자가 추가될 수 있어 게임 공정성이 훼손됨. `app.py:499` 조건을 `if room.status in ('ended', 'paused', 'active')` 로 수정하거나, `waiting` 상태일 때만 입장을 허용하도록 반전(`if room.status != 'waiting' and ...`) 처리. 후자가 더 명시적이며 미래 상태 추가 시 안전.

- **`host_adjust()` `note` 필드 길이 검증 없음 — DB 오버플로 위험** (`app.py:689`): `note = d.get('note', '진행자 자산 조정')` 가 그대로 `RoomTransaction.note` 컬럼(`models.py:78`, `db.Column(db.String(200))`)에 저장됨. SQLite는 `VARCHAR(200)` 를 강제하지 않아 긴 문자열도 저장되지만, PostgreSQL 마이그레이션 시 `STRING(200)` 초과 문자열이 `DataError` 를 일으켜 500 에러 발생. `app.py:689` 바로 다음에 `note = note[:200]` 한 줄 추가로 안전하게 절삭. `host_adjust()` 외에도 동일 패턴이 있는지 전체 `note` 필드 기록 지점 일괄 점검 권장.

- **`loadChart()` 에서 매 탭 클릭마다 Chart.js 인스턴스 파괴 후 재생성** (`static/js/app.js:1403-1404`): `if (S.stockChart) S.stockChart.destroy()` 후 `new Chart(...)` 를 실행하는 방식은 DOM 정리 + 캔버스 재초기화 + 전체 옵션 재파싱을 매번 반복함. 호스트 바 차트(`renderHostBarChart()`, 인근 코드)는 이미 인스턴스가 있으면 `chart.data.labels`, `chart.data.datasets[0].data` 를 교체 후 `chart.update()` 를 호출하는 효율적인 패턴을 사용. `loadChart()` 도 같은 방식으로: 기존 인스턴스가 있고 같은 타입이면 `S.stockChart.data.labels = labels`, `S.stockChart.data.datasets[0].data = closes`, `S.stockChart.data.datasets[0].borderColor = color`, `S.stockChart.update()` 로 대체. 탭 전환 시 차트 깜빡임이 사라지고 렌더링 속도가 빨라짐. 약 10줄 수정.

## 2026-08-14

### 추가하면 좋을 기능

- **주식 매수/매도 수수료 설정 기능** (`models.py:33`, `app.py:464-491` `create_room()`, `app.py:818-863` `trade()`): 현재 거래 시 수수료가 전혀 없어 실제 주식 투자의 핵심 비용 개념을 체험할 수 없음. `Room` 모델에 `fee_rate = db.Column(db.Float, default=0.0)` 컬럼을 추가하고 방 생성 폼에 "거래 수수료 (%)" 입력란(권장 범위 0~1%)을 추가. `trade()` 내 매수·매도 공통 경로에서 `fee = round(amount * room.fee_rate / 100)` 를 계산해 현금에서 추가 차감하고 `RoomTransaction.note` 에 "수수료: 1,000원"으로 기록. 수업 시나리오에서 단기 잦은 거래(수수료 누적) vs. 장기 보유(수수료 1회) 비용 차이를 직접 체험하게 되어 회전율·거래 비용 교육 효과가 생김. DB 컬럼 1개 + 백엔드 5줄 + 프런트 입력 1개 수정.

- **참가자 순위 탭 자동 폴링** (`static/js/app.js` `showPage()` 및 `loadRankings()` 인근): 시장 탭은 10초마다 `S.pollInterval` 으로 자동 갱신되지만 순위 탭(`pg-rankings`)은 클릭 시 1회만 로드됨. 학생이 자신의 순위 변화를 확인하려면 탭을 계속 오가거나 수동으로 새로고침해야 함. `showPage('rankings')` 호출 시 `if (!S.rankInterval) S.rankInterval = setInterval(loadRankings, 8000)` 을 시작하고, 다른 탭(`showPage()` 공통 핸들러)으로 전환 시 `clearInterval(S.rankInterval); S.rankInterval = null` 로 정리하면 됨. `loadRankings()` 는 이미 구현되어 있으므로 클라이언트 약 6줄만 추가하면 되며 서버 변경은 없음. 수업 중 학생들이 순위 탭을 열어두고 실시간으로 경쟁 상황을 확인하는 교육적 긴장감을 유지하는 데 효과적.

- **게임 종료 후 결과 화면에 섹터별 인기 종목 통계 표시** (`app.py:1474-1542` `export_rankings()`, `app.py:1441-1451` `host_publish_results()`, `static/index.html:616-657` 결과 화면): 현재 결과 화면은 개인 순위·자산 바 차트만 표시함. `_compute_leaderboard(rid)` 실행 후 `db.session.query(RoomTransaction.symbol, db.func.count()).filter_by(room_id=rid, action='BUY').group_by(RoomTransaction.symbol).order_by(db.func.count().desc()).limit(5).all()` 로 "가장 많이 사고 팔린 종목 Top5"를 집계하고, 섹터별 총 거래 건수(`STOCKS[sym]['sector']` 매핑 활용)도 계산. 결과 API 응답에 `stats` 필드로 추가하고 결과 화면 하단에 "🏭 섹터별 관심도" 간단 막대 그래프를 삽입. 엑셀 다운로드에도 "거래 통계" 시트 추가하면 수업 마무리 토론 자료가 자동으로 완성됨. 교사의 수업 후 정리 시간 대폭 절감.

- **진행자 대시보드 상단에 현재 시각(KST) 표시** (`static/index.html:115-133` 호스트 topbar, `app.py:123` KST 상수): 서버는 UTC 기준 `end_time` 을 저장하고 프런트는 `remaining_seconds` 카운트다운만 표시하므로 학생들이 "몇 시에 끝나요?" 질문 시 진행자가 직접 계산해야 함. 호스트 상단바에 `<div id="host-clock"></div>` 를 추가하고, `enterHostGame()` 에서 `setInterval(() => { document.getElementById('host-clock').textContent = new Date().toLocaleTimeString('ko-KR'); }, 1000)` 한 줄을 실행하면 됨. 서버 변경 없이 클라이언트 2줄 추가. 진행자가 빔프로젝터에 대시보드를 띄워두면 학생 전체가 현재 시각과 남은 시간을 동시에 확인 가능.

- **게임 시작 전 참가자 대기 화면에 게임 설정 정보 표시** (`static/index.html:346-352` `screen-p-lobby`, `static/js/app.js` `enterParticipantLobby()` 인근): 참가자 로비 화면의 `<div id="plobby-settings">` 요소가 HTML에 존재하지만(`index.html:350`) 현재 `enterParticipantLobby()` 에서 이 요소에 아무것도 채우지 않음. 학생이 로비에서 "시작 자금이 얼마예요?", "몇 분짜리 게임이에요?" 를 선생님에게 개별 질문함. `S.room.starting_cash`, `S.room.duration_minutes`, `S.room.deposit_rate` 값을 받아 `plobby-settings` 에 "💰 시작 자금: 1,000만 원 | ⏱ 30분 | 🏦 금리 3%" 한 줄로 표시하면 됨. 프런트 3줄 수정으로 진행자·학생 간 불필요한 질문이 사라짐.

### 제거/단순화할 것들

- **`api.get()` / `api.post()` / `api.del()` 에서 서버 에러 메시지 소실** (`static/js/app.js:33-44`): `if (!r.ok) return {error: \`HTTP ${r.status}\`}` 처리로 서버가 JSON 바디 `{"error": "잔액 부족 — 필요: 1,000,000원 / 보유: 500,000원"}` 를 반환해도 무시하고 "HTTP 400" 문자열만 반환함. 학생이 매수/매도 실패 시 실제 이유를 볼 수 없어 혼란을 야기하며, 교사도 원인 진단이 어려움. 수정: `if (!r.ok) { try { const e = await r.json(); return {error: e.error || 'HTTP ' + r.status}; } catch { return {error: 'HTTP ' + r.status}; } }` 패턴으로 교체. `api.get`, `api.post`, `api.del` 세 함수 각 3줄 수정. 이 단일 수정으로 `trade()`, `create_deposit()`, `withdraw_deposit()` 등 모든 에러 경로의 상세 메시지가 학생 화면까지 정확히 전달됨.

- **`get_transactions()` / `host_member_transactions()` 에서 룰렛·복권·퀴즈 거래가 '자산조정'으로 오표시** (`app.py:929`, `app.py:715-716`): `'name': STOCKS.get(t.symbol,{}).get('name', '자산조정') if t.action != 'ADJ' else '자산조정'` 로직에서 룰렛(`symbol='ROULETTE'`, `action='RLT'`), 복권(`symbol='LOTTO'`), 퀴즈(`symbol='QUIZ'`), 예금정산(`symbol='DEPOSIT'`) 은 모두 `STOCKS` 딕셔너리에 없어 기본값 `'자산조정'`으로 표시됨. 학생이 거래 내역을 보면 룰렛 이익/손실이 진행자 조정과 동일하게 보여 혼란스러움. 해결: `app.py` 상단에 `SPECIAL_SYMBOLS = {'ROULETTE': '🎰 룰렛', 'LOTTO': '🎟 복권', 'QUIZ': '🧠 퀴즈', 'DEPOSIT': '🏦 예금', 'ADJ': '📋 자산조정'}` 딕셔너리를 추가하고, 두 곳의 name 계산을 `SPECIAL_SYMBOLS.get(t.symbol) or STOCKS.get(t.symbol, {}).get('name', t.symbol)` 로 교체. 총 2개소, 각 1줄 수정.

- **`join_room()` 이 게임 진행 중(`active`) 상태에서 신규 참여자 추가 허용** (`app.py:498-507`): `room.status == 'ended'` 만 차단하므로 방 코드를 아는 학생은 게임이 이미 시작된 후에도 처음 참여 가능. 뒤늦게 입장한 학생은 `starting_cash` 전액을 받아 순위에 포함되어 다른 학생들에게 불공평하며, 교사가 의도치 않은 인원 변화를 겪게 됨(반면 이미 등록된 학생은 재입장이 허용되어야 함). `app.py:501` 의 조건을 `if room.host_id != user.id and not RoomMember.query.filter_by(room_id=room.id, user_id=user.id).first():` 블록 안에서, `if room.status != 'waiting': return jsonify({'error': '대기 중인 방에만 새로 참여할 수 있습니다.'}), 400` 를 추가하면 기존 멤버 재접속은 허용하면서 게임 중 신규 입장만 차단할 수 있음. 3줄 추가.

- **`User.username` 고유 제약으로 다른 방의 동명이인 학생이 타인 계정으로 로그인** (`models.py:19`, `app.py:435-442`): `User.username = db.Column(db.String(50), unique=True)` + 저장 형식 `"{학번} {이름}"`. 서로 다른 반의 학생이 같은 학번·이름을 가진 경우(예: 두 반 모두 학번 "20715" 김민준), 두 번째 학생이 접속하면 `User.query.filter_by(username=u).first()` 로 **첫 번째 학생의 계정으로 로그인**되어 그 학생의 포트폴리오를 볼 수 있고 거래까지 가능함. `app.py:437-441` 의 로그인 로직에 방 코드 검증을 추가하거나, 단기적으로 `User.username` 에 타임스탬프·랜덤 suffix를 붙여 새 계정을 강제 생성하도록 수정. 근본 해결은 `User` 모델에 `student_id` 컬럼을 분리하고 방별 고유성만 보장하도록 스키마 재설계.
...
- **종목별 실제 인게임 가격 이력 적재 및 차트 반영** (`stock_service.py:303-332`, `StockService._init_prices()`, `StockService.get_price()`): `get_history()`는 현재 `random.gauss()`로 완전히 가짜 OHLCV 바를 생성하며, 캐시 만료 시마다 형태가 달라진다. `get_price()` 호출 시마다 `(timestamp, price)` 튜플을 `self._price_log: list[tuple[float, float]] = []`에 추가(최대 500개)하고, `get_history()`에서 이 로그를 바탕으로 분봉/일봉을 집계하면 실제 게임 흐름을 반영한 일관된 차트를 제공할 수 있다. 서버 약 25줄, 클라이언트 변경 없음. 학생이 "내가 매수한 직후 주가가 얼마나 올랐나"를 차트로 확인하는 교육 경험을 제공.

- **진행자 대시보드: 섹터별 평균 수익률 요약 패널** (`app.js:316-360`, `loadHostMarket()`): 현재 호스트 시장 탭은 종목 카드를 단순 나열하므로 60개 종목 전체를 스크롤해야 시장 흐름 파악이 가능하다. `/api/rooms/<rid>/stocks` 응답에는 이미 `sector` 필드가 있으므로, `loadHostMarket()` 상단에 `data.stocks.reduce()`로 섹터별 `change_pct` 평균을 계산해 `<div class="sector-summary">` 그리드(예: "반도체 +2.3% ↑ / 바이오 -1.1% ↓ …")를 종목 카드 위에 삽입하면 된다. 서버 변경 불필요, 클라이언트 약 15줄. 교사가 수업 중 "지금 어느 섹터가 강세냐"는 질문에 즉각 응답 가능.

- **학생별 매매 한도 설정 (종목당 최대 보유 비율)** (`app.py:818-863`, `trade()`): 현재 `trade()` 에는 1회 주문 수량 상한이 없어 학생이 시작 자금 전액을 단일 종목에 몰아넣을 수 있다. `room.max_holding_pct FLOAT` 컬럼(기본값 `100.0`)을 `Room` 모델(`models.py:25`)에 추가하고, `trade()` BUY 처리부에서 `total = member_total_value(rid, uid); current_holding = holding.shares * price if holding else 0; if (current_holding + amount) / total > room.max_holding_pct / 100: return 400`을 삽입하면 된다. 진행자 설정 탭(`index.html:211`)에 슬라이더 추가. 분산 투자 개념을 강제로 체험시키는 교육 효과 있음.

- **게임 일시정지 중 퀴즈 기능 허용** (`app.py:1318-1389`, `get_quiz()`, `submit_quiz()`): 두 엔드포인트 모두 `if room.status != 'active': return 400` 으로 거부하므로, 복권 또는 룰렛 진행 중 게임이 `paused` 상태가 되면 학생들은 대기 시간에 퀴즈를 풀 수 없다. 조건을 `if room.status not in ('active', 'paused'): return 400`으로 완화하면 복권 대기(60초)·룰렛 대기 시간에 학생들이 퀴즈를 활용할 수 있어 유휴 시간이 줄어든다. 서버 2줄 수정, 클라이언트 변경 없음.

- **진행자 관리 화면: 학생 강퇴 기능을 게임 진행 중에도 허용** (`app.py:651-662`, `kick_member()`): `room.status != 'waiting'` 조건 때문에 게임 시작 후에는 강퇴가 불가하다. 게임 진행 중 강퇴 시 해당 학생의 보유 주식을 현재가로 청산(`_end_room()` 내 정산 로직과 동일)하고, `RoomHolding`·`Deposit`을 제거한 뒤 `RoomMember` 레코드를 삭제하도록 로직을 확장하면 된다. 학생이 실수로 잘못된 이름으로 입장한 경우 재입장 처리를 위한 필수 운영 기능.

- **자산 변화 꺾은선 그래프를 참가자 포트폴리오 탭에 노출** (`app.js:738-756`, `refreshMyRank()`): `S.assetHistory` 배열은 이미 `{label, value}` 형태로 10초마다 적재되고 있으나(`app.js:751-756`), 데이터가 화면에 출력되는 곳이 없다. 포트폴리오 탭(`pg-portfolio`)에 `<canvas id="asset-line-chart">` 를 추가하고 `S.assetLineChart`(이미 `S` 객체에 선언됨, `app.js:7`)를 Chart.js `line` 타입으로 초기화·업데이트하면 된다. 서버 변경 불필요, 약 20줄. 학생이 자신의 수익 곡선을 실시간으로 보며 거래 결정 시점과 자산 변화의 상관관계를 직관적으로 학습.

### 제거/단순화할 것들

- **`_compute_leaderboard()` N+1 쿼리 문제** (`app.py:151-171`, `member_total_value()`): `_compute_leaderboard()`는 참가자 수만큼 `member_total_value()`를 반복 호출하고, 각 호출은 `RoomHolding.query.filter_by(...).all()` 및 `Deposit.query.filter_by(...).all()` 쿼리를 따로 실행한다. 학생 30명이면 최소 60회의 개별 SELECT가 발생한다. 이 함수는 `/api/rooms/<rid>/rankings` (10초 폴링), `/api/rooms/<rid>/host/members` (10초 폴링), `export_rankings()` 등 여러 경로에서 호출된다. `RoomHolding.query.filter_by(room_id=rid).all()`과 `Deposit.query.filter_by(room_id=rid, status='active').all()`로 방 전체 데이터를 1회씩 일괄 조회한 뒤 `user_id`를 키로 딕셔너리에 그루핑하면 SELECT 횟수를 3~4회로 줄일 수 있다.

- **`Room.query.get_or_404(rid)` 는 Flask-SQLAlchemy에서 deprecated** (`app.py:533`, `app.py:576` 외 다수): `Room.query.get_or_404(rid)` 패턴은 Flask-SQLAlchemy 3.x에서 deprecated됐으며, `db.get_or_404(Room, rid)` 또는 `db.session.get(Room, rid)` + 수동 404 처리가 권장된다. `app.py` 전체에 약 20개 사용처(`grep "get_or_404" app.py`로 확인 가능). 동작에는 지장 없지만 향후 버전 업그레이드 시 DeprecationWarning이 발생할 수 있어 한 번에 일괄 치환 가능.

- **`_member_locks` 딕셔너리 무한 증가** (`app.py:109-119`, `_get_member_lock()`): `_member_locks`는 `(room_id, user_id)` 쌍마다 `threading.Lock()` 을 생성하며, `_end_room()` 에서만 해당 방의 키를 정리한다(`app.py:259-261`). 방이 많아지면 메모리가 누수되고, 특히 Render 무료 플랜에서 인스턴스가 재시작되지 않고 장기 실행되는 경우 문제가 될 수 있다. `_end_room()` 외에도 `create_room()` 에서 stale 방 정리 후 관련 키를 `_member_locks_meta`로 동일하게 정리하는 코드를 추가하거나, 현재 `ended`가 아닌 방의 uid만 유지하는 주기적 정리 루틴을 추가해야 한다.

- **`quiz_settings`·`roulette_config`이 서버 재시작 시 기본값으로 초기화** (`app.py:1314-1316`, `app.py:351-352`): Render 무료 플랜은 비활성화 후 재시작(cold start)이 잦은데, 진행자가 설정한 퀴즈 보상/패널티 비율과 룰렛 확률이 모두 인메모리 딕셔너리에만 저장돼 재시작 시 기본값으로 돌아간다. `Room` 테이블에 `quiz_reward_pct FLOAT DEFAULT 1.0`, `quiz_penalty_pct FLOAT DEFAULT 0.5` 컬럼을 추가(`models.py`)하고 엔드포인트에서 DB에 읽고 쓰도록 변경. 룰렛 설정도 `room.rlt_config JSON` 컬럼 또는 별도 컬럼으로 영속화. 수업 중 예기치 않은 설정 초기화를 방지.
...
- **진행자 공지 메시지 브로드캐스트** (`app.py` 신규 `POST /api/rooms/<rid>/host/announce`, `GET /api/rooms/<rid>/announce`): 현재 진행자가 학생 전원에게 텍스트 메시지를 보낼 방법이 없어 뉴스 이벤트(stock_service.py:163-180) 외에 별도 안내를 할 수단이 없음. `_announcements: dict = {}  # room_id -> {'text': str, 'ts': float}`를 in-memory로 추가하고, 학생 폴링 시 `ts`를 비교해 새 공지를 toast로 표시하면 됨. 참가자 `GET /api/rooms/<rid>` 폴링(app.js에서 2초 간격)에 `announce_ts`를 함께 내려보내거나 별도 엔드포인트로 분리해도 됨. 서버 약 15줄, 클라이언트 toast 표시 10줄. 교사가 수업 중 "지금 반도체 섹터에 주목하세요" 등 실시간 교육 안내 가능.

- **거래 수수료(증권거래세) 설정** (`app.py:842-887` `trade()`, `_quiz_settings` 패턴 참고): 현재 매수·매도 모두 수수료가 없어(`amount = price * shares`만 차감; app.py:863) 실제 증권거래에 존재하는 비용 개념을 가르칠 수 없음. `_trade_settings: dict = {}`에 `buy_fee_pct`(0~1%), `sell_fee_pct`(0~0.5%) 를 추가하고 `trade()` 내 `amount` 계산 후 `fee = round(amount * fee_pct / 100)` 를 차감. 기본값 0(현행 동일)으로 설정해 하위 호환성 유지. 진행자 설정 탭에 슬라이더 UI 추가. 서버 약 20줄. "왜 단타 매매는 비효율적인가" 수업에 직결.

- **포트폴리오 섹터 분산도 경고** (`app.py:892-923` `get_portfolio()`, `static/js/app.js` holdings 렌더 부분): 현재 포트폴리오 API는 개별 종목 정보를 반환하지만 섹터 집중도를 계산하지 않음. `get_portfolio()` 응답에 `sector_concentration: {sector: pct}` 딕셔너리를 추가(기존 holdings 루프 내에서 섹터별 `current_value` 합산)해 특정 섹터 비중이 60% 이상이면 클라이언트에서 ⚠️ 경고 배지를 표시. 서버 10줄 추가(기존 holdings 루프 확장), 클라이언트 UI 5줄 추가. "분산투자" 개념을 즉각적인 시각 피드백으로 교육.

- **결과 화면 공유 가능한 URL** (`app.py` 신규 `GET /results/<room_code>`, `static/index.html`): 게임 종료 후 `screen-results`(index.html:617)는 세션에 로그인한 사람만 볼 수 있어, 진행자가 결과를 수업 슬라이드에 띄우거나 부모님께 공유하기 어려움. `GET /results/<room_code>` 라우트를 추가해 종료된 방의 최종 순위(`_compute_leaderboard()`)를 별도 HTML 페이지로 렌더링(로그인 불필요, results_published=True인 경우만 공개). 서버 약 15줄, 결과 발표 시 공유 URL 자동 생성 후 복사 버튼 노출.

- **매도 후 쿨다운(단타 제한)** (`app.py:842-887` `trade()`, `_member_locks` 패턴 참고): 실제 주식 시장에서 동일 종목을 매도 직후 재매수하는 "당일 매매 제한(PDT Rule)" 또는 "재매수 대기 시간" 개념을 가르치기 위해, `_sell_cooldown: dict = {}  # (rid, uid, symbol) -> sell_ts` 를 추가해 매도 후 `N`초 동안 동일 종목 재매수를 차단하는 선택적 설정을 제공. 진행자가 `POST /api/rooms/<rid>/host/trade-settings`에서 `sell_cooldown_secs: int` 를 0(기본, 현행 동일)~300초 범위로 설정 가능. 서버 약 20줄, UI 설정 5줄.

### 제거/단순화할 것들

- **`_liquidate_shortfall()`이 정기예금을 강제 해지** (`app.py:213-228`): 룰렛 베팅(`app.py:1150-1151`)과 퀴즈 오답 패널티(`app.py:1405-1406`)에서 `_liquidate_shortfall()`이 호출될 때, `lock_type='fixed'`인 정기예금(`models.py:90`)도 `dep.status = 'withdrawn'`으로 바꿔 강제 해지함(app.py:217). 이는 `withdraw_deposit()` 엔드포인트(app.py:1030)에서 `if dep.lock_type == 'fixed': return error`로 정기예금 해지를 금지한 규칙과 정면 충돌. `_liquidate_shortfall()` 내 예금 루프(app.py:213)에 `if dep.lock_type == 'fixed' and not force_fixed: continue` 조건을 추가하고, `credit_cash=True`(룰렛)일 때만 `force_fixed=True`를 전달하거나, 아니면 정기예금은 어떤 경우에도 강제 청산 대상에서 제외하도록 정책을 통일해야 함.

- **`get_history()` 차트 데이터가 랜덤으로 재생성되어 교육 혼란 유발** (`stock_service.py:303-332`): 종목 차트 히스토리를 매번 `random.gauss()`로 새로 생성(`stock_service.py:321-327`)하고, 캐시 TTL이 120초(stock_service.py:124)여서 같은 날 '1달' 차트를 2분 후 다시 보면 완전히 다른 그래프가 나옴. 학생이 "어제도 이 주식이 올랐어" 라고 기억했던 차트가 사라지는 혼란이 생김. 해결책: `StockService.__init__()` 시점에 모든 종목의 히스토리를 한 번만 생성해 `self._history_static`에 저장하고 이후 `get_history()` 호출 시 static 데이터를 기반으로 최신 가격을 이어붙이는 방식으로 변경. 캐시 TTL 무한으로 설정하거나 종목 최신 가격 변경 시 히스토리 마지막 값만 갱신. 약 20줄 수정.

- **`force_price()` 가격 클램프 범위가 `_next_price()`와 불일치** (`stock_service.py:247`, `stock_service.py:161`): `_next_price()`는 `max(base * 0.6, min(base * 1.4, new_price))`로 클램핑(stock_service.py:161)하지만, `force_price()`는 `max(base * 0.3, min(base * 3.0, new_price))`로 2.5배 넓은 범위(stock_service.py:247)를 허용. 진행자가 +50% 강제 조정을 하면 가격이 `base * 1.5`로 올라갔다가, 다음 정기 가격 업데이트에서 `_next_price()`의 `base * 1.4` 상한에 즉시 클램핑되어 뚝 떨어지는 현상 발생. 두 함수 모두 동일한 상수(`MAX_MULT = 1.4`, `MIN_MULT = 0.6` 또는 `MAX_MULT = 3.0`, `MIN_MULT = 0.3`)를 모듈 레벨 상수로 정의해 공유하도록 단일화.

- **`goHome()` 호출 시 항상 세션 로그아웃** (`static/js/app.js:111-117`): `goHome()` 함수가 `/api/auth/logout`을 무조건 호출해 로그아웃 처리를 함. 결과 화면 "홈으로" 버튼(index.html:655), 룰렛 종료 후 자동 결과 화면 이동 등 여러 경로에서 호출됨. 학생이 실수로 "홈으로"를 누르면 세션이 완전히 만료돼 재접속 시 방 코드를 다시 입력해야 함. `find_active_room()`이 종료된 방을 반환하지 않으므로, 로그아웃 없이 `showLanding()`만 호출해도 충분함. `goHome()` 내 `api.post('/api/auth/logout', {})` 라인(app.js:112) 제거 또는 진행자·참가자 역할을 확인해 호스트만 로그아웃 유도.
...
- **매수 직후 N초 이내 취소 기능** (`app.py:1081-1134` `trade()`, 인메모리 `_trade_hold: dict` 신규 추가): 학생이 실수로 잘못된 종목이나 수량을 매수했을 때 즉시 취소할 방법이 없어, 직후 매도를 해도 스프레드만큼 손해를 보게 됨. `_trade_hold: dict = {}  # (rid, uid, symbol, ts) -> {shares, price, txn_id}` 딕셔너리에 BUY 완료 시각을 기록하고 `POST /api/rooms/<rid>/cancel-trade {symbol}`에서 5초 이내 요청이면 동일 가격으로 롤백 + 생성된 `RoomTransaction` 레코드 삭제. 클라이언트는 매수 성공 toast에 5초 카운트다운 "↩ 취소" 버튼을 함께 표시하고 시간 만료 시 자동 제거. `_trade_hold` 키는 5초 경과 후 자동 만료로 메모리 관리. 서버 약 25줄, 클라이언트 toast 확장 약 12줄. 수업 중 실수 복구로 학생 불만 방지, 교사 개입 부담 감소.

- **순위표에서 내 앞뒤 2순위 고정 표시** (`app.py:262-290` `_compute_leaderboard()`, `app.js` 순위 탭 렌더링): 학생이 15위일 때 전체 30명 목록을 스크롤해야 자신의 순위를 찾을 수 있어 경쟁 긴장감이 약화됨. 순위 탭 상단에 "내 주변 순위" 고정 섹션을 추가해 내 앞 2명·나·뒤 2명(총 5개 행)을 항상 먼저 표시하고, 전체 순위는 하단 스크롤로 제공. `_compute_leaderboard()` 결과에서 내 `user_id` 기준 `board[max(0, idx-2):idx+3]`를 슬라이싱하면 서버 변경 없이 클라이언트 약 15줄 추가로 구현 가능. "내 바로 위 1등을 앞지르려면 ₩127만원 필요" 같은 동기 유발 문구도 함께 표시하면 경쟁 몰입도 향상 효과 큼.

- **진행자 대시보드 학급 전체 자산 합산 요약 카드** (`app.py:262-290` `_compute_leaderboard()`, `app.py:906-912` `host_members()`): 현재 진행자는 개인별 총자산·수익률만 볼 수 있고, 학급 전체 자산 합계("지금 이 교실의 총 투자 자산이 얼마인가")를 즉시 파악할 방법이 없음. `host_members()` 응답에 `class_stats: {total_assets, gain_pct, above_start_count}` 필드를 추가 — `_compute_leaderboard()` 결과를 합산해 `total_assets = sum(e['total_value'])`, `gain_pct = (total_assets - room.starting_cash * member_count) / (room.starting_cash * member_count) * 100`을 계산. 진행자 순위 탭 상단에 "학급 총 자산: ₩3억 2천만 (+8.5%)" 요약 카드 표시. "복권·퀴즈 보상이 학급 총 자산을 어떻게 팽창시키는가"를 인플레이션 개념 수업 포인트로 활용 가능. 서버 약 8줄, 클라이언트 카드 렌더링 약 10줄.

- **참가자 순위 탭 "현재 1등까지 격차" 표시** (`app.py:262-290` `_compute_leaderboard()` 응답, `app.js` 순위 탭 학생 행): 학생이 자신의 순위를 볼 때 "1등과 얼마나 차이 나는지"가 없어 추격 동기가 약함. `_compute_leaderboard()` 응답에 `gap_to_first: float` 필드(1등 자산 - 내 자산, 1위면 0)를 추가하고, 학생 본인 행 오른쪽에 "▲₩240만 더 필요" 배지를 빨간색으로 표시. 1위이면 "🏆 선두 유지 중" 메시지로 대체. 서버 `_compute_leaderboard()` 리턴 딕셔너리에 필드 한 개 추가(2줄), 클라이언트 배지 렌더링 약 8줄. 막판 역전 드라마를 유도해 수업 집중도 향상 직결.

- **게임 종료 결과 화면 "주요 순간" 하이라이트 카드** (`app.py:335-388` `_end_room()`, 인메모리 `_room_events: dict` 신규 추가): 게임이 끝나고 결과 화면에는 최종 순위만 있고 "어떤 이벤트가 승부를 갈랐는지" 스토리가 없어 수업 회고 포인트가 약함. `_room_events: dict = {}  # rid -> [{type, desc, ts}]` 딕셔너리에 복권 당첨(`_do_reveal()`, `app.py:463-512`)·룰렛 고배율 당첨(`minigame_spin()`, `app.py:1350-1411`)·퀴즈 정답/오답 패널티·진행자 강제 주가 조정 이벤트를 최대 20건까지 기록하고, `GET /api/rooms/<rid>/events` 엔드포인트로 결과 화면에 타임라인 카드 렌더링. `_end_room()`에서 해당 딕셔너리 정리 필요. 서버 약 20줄(기록 + API), 클라이언트 타임라인 카드 렌더링 약 20줄. "룰렛 25배 당첨으로 이철수 일등 도약" 같은 수업 회고 스토리텔링 직결.
...
- **`_push_notified` 에 방 종료 후에도 해당 방 키가 잔류 — 5000 임계치 조기 도달 유발** (`app.py:335-387` `_end_room()`, `app.py:149` `_push_notified`, `app.py:217-218` 전체 clear 로직): `_end_room()`은 `_lots`, `_rlt_active`, `_member_locks` 등을 방별로 정리하지만 `_push_notified` 세트는 정리하지 않음. 복권/룰렛 이벤트마다 `(rid, 'lot'|'rlt', ts)` 키가 추가되는데, 방 종료 후에도 이 키들이 세트에 잔류해 5000개 임계치 도달(`app.py:217`)을 앞당김. 임계치 초과 시 `_push_notified.clear()`가 실행돼 진행 중인 다른 방의 알림 키까지 삭제되어 중복 발송 위험이 가중됨(이 부작용은 기존 2026-08-28 항목에서 지적됐으나 근본 원인인 방 종료 시 선제 정리 미구현은 미지적). `_end_room()` 마지막에 `_push_notified -= {k for k in _push_notified if k[0] == room.id}` 1줄 추가로 해결.

- **`_compute_leaderboard()` 에서 `Deposit` 전체 조회 후 Python 레벨 status 필터 — 불필요한 데이터 전송** (`app.py:272-274`, `_compute_leaderboard()`; `app.py:257-259`, `member_total_value()`): `Deposit.query.filter_by(room_id=rid).all()` (line 273)는 `status` 필터 없이 `withdrawn`·`matured` 예금까지 전부 조회한 뒤, `member_total_value()` (line 258)에서 `if d.status == 'active':` Python 조건으로 걸러냄. 게임 중 해지·만기된 예금이 많을수록 DB에서 전송되는 행 수가 늘어남. `Deposit.query.filter_by(room_id=rid, status='active').all()` 로 단 1자 조건 추가로 불필요한 행 제거 가능. 게임 내 예금 해지가 빈번한 경우(자유입출금 반복) 특히 효과가 큼. 1줄 수정, 기능 변화 없음.

- **`_get_room_cached()` 동시 캐시 미스 시 `room_dict()` 중복 계산 — Thundering Herd** (`app.py:92-105` `_get_room_cached()`): 캐시 TTL(1.5초) 만료 직후 여러 학생 폴링이 동시에 도달하면 캐시 읽기 락을 해제한 뒤 각자 `room_dict(room, uid)` (line 101)를 독립적으로 계산하고 모두 캐시에 쓰는 중복 계산이 발생. 30명 학생이 10초 폴링 시 이론상 모든 만료 순간에 최대 30건의 `room_dict()` 계산이 동시에 실행될 수 있음. 단순 해결책: `with _room_cache_lock:` 블록 안에서 재확인(double-checked locking) 후 캐시 미스일 때만 락 유지하며 `room_dict()` 계산. 정확성 문제는 아니나 Render 무료 플랜 CPU 한계 환경에서 20명+ 동시 폴링 시 응답 지연을 줄이는 효과. 약 6줄 구조 변경.

- **`host_roulette_config()` POST 성공 후 GET 응답과 실제 룰렛 확률 불일치 — UI 혼동** (`app.py:1683-1701` `host_roulette_config()`, `app.py:524-532` `_rlt_outcomes()`): GET 응답은 저장된 raw `weights` 배열(`[700, 200, 70, 20, 10]`)을 그대로 반환하지만, 실제 룰렛 확률은 `_rlt_outcomes()` 에서 `total = sum(weights)`로 정규화해 `[70%, 20%, 7%, 2%, 1%]`를 사용. 클라이언트가 raw weights를 퍼센트로 오인하면("700%??") 진행자가 설정이 잘못됐다고 착각할 수 있음. 인덱스별 `outcomes[i]` 진행 확률을 GET 응답에 포함하거나 (`pct = w/sum(weights)*100`), UI에서 합계 기준 정규화 후 표시하도록 명확히 해야 함. 서버 GET 응답에 `probs: [70, 20, 7, 2, 1]` 필드 추가(1줄), 클라이언트 설정 UI에서 raw weights 대신 `probs`로 표시 전환.
...
- **진행자 방송 메시지 기능** (`app.py:562-580` `room_dict()`, `static/js/app.js:1311-1338` `refreshRoomStatus()`): 진행자가 수업 중 "지금 배터리 섹터를 주목하세요!"와 같은 안내 메시지를 전체 학생 화면에 즉시 팝업으로 전달할 수단이 없어 구두로만 지시해야 함. `Room` 모델(`models.py:25-44`)에 `host_broadcast = db.Column(db.String(200), nullable=True)` 컬럼 추가, `room_dict()` 응답에 `host_broadcast` 필드 포함, `POST /api/rooms/<rid>/host/broadcast {'message': '...'}` 엔드포인트(진행자 전용, 5줄) 추가. 학생 화면의 `refreshRoomStatus()` 또는 10초 폴링에서 `r.host_broadcast` 변화 감지 시 `showBombNews()` 스타일로 3초 팝업 표시. 서버 측 메시지는 5분 후 자동 소멸(`host_broadcast=None`)하도록 `broadcast_until` 타임스탬프와 함께 관리. 전체 변경 약 30줄, 스키마 마이그레이션 1줄.

- **관심 종목 목표가 도달 시 인앱 알림** (`static/js/app.js:16-17` `S.watchlist`, `static/js/app.js:1360-1371` `loadMarket()`): 학생이 관심 종목으로 등록(`☆` 버튼)해도 목표 가격 도달 여부를 능동적으로 알 방법이 없어 매 주기마다 직접 확인해야 함. `localStorage`에 `watchlistAlerts = {'NVDA': {up: 200000, down: 150000}}` 형식으로 상·하단 목표가 저장 UI를 관심 종목 카드에 추가(연필 아이콘 클릭 → 입력). `loadMarket()` 완료 후 기존 `prevPrices`와 현재가를 비교해 목표가 돌파 시 `toast('🎯 NVIDIA 목표가 200,000원 돌파!', 'success')` + 진동(`navigator.vibrate`) 호출. 서버 변경 없이 `app.js` 약 40줄 + CSS 아이콘 스타일 5줄만으로 구현. 학생이 목표가를 미리 설정하고 이유를 적으면 투자 가설 검증 활동으로 확장 가능.

- **거래 시 투자 메모 입력 — 기존 `note` 컬럼 활용** (`models.py:78` `RoomTransaction.note`, `app.py:1122-1123` `trade()`): `RoomTransaction.note`가 `nullable=True`로 선언돼 있으나 `trade()` 엔드포인트에서 `note=None`으로 고정되어 학생이 "왜 이 종목을 샀는가"를 기록할 수 없음. 프론트 거래 모달(`openStockModal()`)에 선택 입력 필드 `<input placeholder="투자 이유 (선택)" maxlength="80">` 추가 후 `api.post('/trade', {symbol, action, shares, note})` 에 포함. `trade()` 서버에서 `note = d.get('note','')[:80]` 로 50줄 0자 추가. `get_transactions()` 응답에도 이미 `note` 필드 포함(`app.py:1194`)돼 있으므로 거래 내역 탭에서 학생 본인 메모를 즉시 확인 가능. 수업 후 투자 일지 작성·복기 활동에 바로 활용할 수 있는 영향 대비 구현 비용이 매우 낮은 기능.

- **섹터별 학급 자금 배분 히트맵** (`app.py:262-290` `_compute_leaderboard()`, `models.py:57-65` `RoomHolding`): 진행자가 "학급 전체 자금이 어떤 섹터에 집중되어 있나"를 히트맵으로 볼 수단이 없어 분산투자 교육 포인트를 만들기 어려움. `GET /api/rooms/<rid>/host/sector-heat` 엔드포인트 추가 — `RoomHolding`과 `StockService.get_price()` 를 순회해 `{sector: 총 평가금액, pct: 비중}` 목록 반환(약 15줄). 진행자 Market 탭에 섹터별 가로 막대 차트(Chart.js 기존 import 활용) 추가. "반도체에 60% 집중" 시각화 → "왜 분산이 중요한가" 토론 트리거. 기존 인프라만으로 서버 약 15줄 + 클라이언트 약 25줄.

- **게임 중간 하프타임 스냅샷 및 중간 순위 공개** (`app.py:1735-1745` `host_publish_results()`, `app.py:1166-1175` `get_rankings()`): 게임이 종료돼야만 순위를 학생에게 공개할 수 있어 중간에 진행 상황을 공유하면서 "지금 전략을 바꿀까?" 토론을 유도하기 어려움. `rooms` 테이블에 `midgame_snapshot_at` 컬럼 추가, `POST /api/rooms/<rid>/host/snapshot` 호출 시 현재 순위를 `rooms.midgame_board` JSON 컬럼(text)에 저장. 학생 화면에서 "중간 순위 보기" 탭이 활성화되어 스냅샷 기준 랭킹과 현재 랭킹 대비 변화(▲2 / ▼3) 확인 가능. 교사가 게임 중 전략적 시점에 "지금 공개!"를 누르면 되는 단순한 플로우. 서버 약 25줄 + 마이그레이션 2줄.

### 제거/단순화할 것들

- **`Room.query.get_or_404(rid)` — 전체 라우트에서 SQLAlchemy 레거시 Query API 사용** (`app.py:760, 811, 827, 837, 853, 876, 909, 915, 927, 940, 988, 1011, 1033, 1047, 1063, 1067, 1081, 1131, 1167, 1180, 1203, 1230, 1262, 1284, 1301, 1328, 1359, 1410, 1446, 1481, 1517, 1541, 1663, 1681, 1704, 1735, 1748, 1768` 등): `Room.query.get_or_404(rid)`는 SQLAlchemy 2.0에서 `LegacyAPIWarning` 대상이며 SQLAlchemy 3.0에서 제거 예정. 올바른 패턴은 `room = db.session.get(Room, rid); if not room: abort(404)` 또는 Flask-SQLAlchemy `db.get_or_404(Room, rid)` (1.4+). `RoomMember.query.filter_by(...).first()`, `.all()`, `.count()` 도 동일하게 `db.session.execute(select(RoomMember).filter_by(...)).scalars()` 형식으로 마이그레이션 필요. 현재는 동작하나 향후 버전업 시 전체 라우트가 동시에 깨질 위험 있음. 한 라우트씩 점진 교체 가능.

- **`get_room()` 내 룰렛 트리거 조건 검사 시 참여자당 COUNT 쿼리 N번 발생** (`app.py:774-779`): `has_spins = any(RoomTransaction.query.filter_by(room_id=rid, user_id=m.user_id, action='RLT').count() < 3 for m in non_host)` 는 학생이 30명이면 30번의 `COUNT(*)` 쿼리가 `GET /api/rooms/<rid>` 폴링(10초마다)마다 실행됨. `db.session.execute(select(RoomTransaction.user_id, func.count()).filter_by(room_id=rid, action='RLT').group_by(RoomTransaction.user_id))` 단일 쿼리로 `{uid: spin_count}` 매핑을 만들면 쿼리 1번으로 축소 가능. 인원이 많을수록 DB 부하가 선형 증가하는 구조.
...
- **`import math` 함수 내부 중복 임포트** (`app.py:1379`): `minigame_spin()` 내부에 `import math` 가 존재하나 `math`는 이미 파일 상단 `app.py:6`에서 `import os, threading, math, re, json, logging`으로 임포트됨. 함수 내부 임포트는 Python 인터프리터가 매 호출마다 `sys.modules` 를 조회해 반환하므로 실제 재임포트는 일어나지 않지만, 중복 선언이 코드를 혼동시키고 상단 임포트 삭제 시 버그로 이어질 수 있음. `app.py:1379`의 `import math` 1줄 삭제로 해결.

- **`SECRET_KEY` 하드코딩 폴백 — 배포 환경에서 세션 위조 가능** (`app.py:24`): `app.secret_key = os.environ.get('SECRET_KEY', 'mock-stock-game-secret-2024')` 는 환경 변수 미설정 시 공개 소스에서 알려진 고정값으로 동작해, 공격자가 해당 값으로 세션 쿠키를 위조해 임의 `user_id`로 API를 호출할 수 있음. Render 배포 시에는 `SECRET_KEY` env var를 설정하도록 `README`에 명시돼 있더라도, 미설정 배포를 코드 수준에서 방지하지 않음. `if not os.environ.get('SECRET_KEY') and not app.debug: raise RuntimeError('SECRET_KEY 환경 변수를 설정하세요.')` 5줄 추가로 프로덕션 실수 방지 가능. 개발 환경 `FLASK_DEBUG=true` 시에는 폴백 허용.
...
- **주요 테이블에 복합 인덱스 추가 (DB 성능)** (`models.py:47-103`): `RoomMember`, `RoomHolding`, `RoomTransaction`, `Deposit` 테이블 모두 `filter_by(room_id=rid, user_id=uid)` 패턴으로 빈번히 조회되지만 `(room_id, user_id)` 복합 인덱스가 없음. `UniqueConstraint`가 선언되어 있어 Postgres에서는 내부적으로 인덱스가 생성되나(`RoomMember`, `RoomHolding`), `RoomTransaction`과 `Deposit`은 UNIQUE 제약 없이 단순 FK만 존재 — 학생 30명이 3초마다 폴링하면 `_compute_leaderboard()`(`app.py:262-290`)가 초당 10회 호출되고 각 호출마다 `RoomTransaction`, `RoomHolding`, `Deposit` 전체를 `room_id` 단일 컬럼으로 스캔. `models.py`의 `RoomTransaction`과 `Deposit` 클래스 `__table_args__`에 `db.Index('ix_rtxn_room_user', 'room_id', 'user_id')` 추가. Render Postgres 환경에서 거래 건수 누적 시 쿼리 속도 수배 개선.

- **게임 중 강퇴(kick) 기능 확장** (`app.py:914-925` `kick_member()`): 현재 `if room.status != 'waiting': return jsonify({'error': '대기 중인 방에서만 강퇴할 수 있습니다.'}), 400` 로 게임 시작 후에는 진행자가 문제 학생을 퇴장시킬 방법이 없음. `active`/`paused` 상태에서도 강퇴를 허용하되, 보유 주식·예금을 현재가로 즉시 청산해 현금으로 환산 후 `RoomMember` 삭제 처리: `waiting` 여부 체크 블록 제거 → `room.status in ('active','paused')` 일 때 `svc.get_price()` 루프로 보유 주식 현금화 + `Deposit` `status='matured'` 처리 후 `db.session.delete(m)`. 수업 중 실수로 잘못 참여한 학생이나 게임 방해 학생을 즉각 대처 가능.

- **`_rlt_active`·복권 현재 회차(`_lots[rid]['current']`) DB 영속화** (`app.py:513-515`, `app.py:390-404`): 룰렛 대기(`rlt_triggered=True`) 또는 복권 진행 중에 Render 무료 티어의 서버 재시작이 발생하면 `_rlt_active`와 `_lots[rid]['current']`가 초기화되어 방이 `paused` 상태로 영구 멈춤. `rlt_triggered`는 이미 DB에 저장되나 `_rlt_active` count는 없음. `Room` 테이블에 `lottery_current_state TEXT DEFAULT NULL` 컬럼 추가(JSON 직렬화), `get_room()` 재진입 시 `room.lottery_current_state`가 있으면 `_lots[rid]['current'] = json.loads(...)` 로 복구. `_do_reveal()` 완료 시 `room.lottery_current_state = None`. 서버 재시작 후에도 복권·룰렛 이벤트가 중단 없이 재개돼 수업 중단 방지.

- **게임 결과 자동 공개 옵션** (`app.py:1735-1745` `host_publish_results()`, `app.py:335-387` `_end_room()`): 게임 종료 후 진행자가 "결과 공개" 버튼을 수동으로 눌러야 참가자 결과 화면이 열림(`results_published` 플래그). 수업 종료 시 진행자가 단말기를 닫거나 학생들을 챙기다 누락하면 학생들은 흰 화면 또는 "결과가 아직 공개되지 않았습니다" 상태로 남음. `Room` 테이블에 `auto_publish BOOLEAN DEFAULT TRUE` 컬럼 추가 (방 생성 시 기본 활성화). `_end_room()` 마지막에 `if room.auto_publish: room.results_published = True` 삽입. 별도 설정 UI 없이도 기본 동작이 "바로 공개"가 되어 학생 혼선 최소화.

- **참가자 폴링을 Server-Sent Events(SSE)로 전환** (`app.py:757-806` `get_room()`, `static/js/app.js` `S.pollInterval` 라인 8): 현재 프론트엔드는 `setInterval`로 `/api/rooms/<rid>` · `/api/rooms/<rid>/stocks` · `/api/rooms/<rid>/news`를 3~5초마다 반복 요청. 학생 30명이면 초당 30~60 HTTP 요청이 Flask 스레드를 점유 — Render 무료 티어(1 CPU, gunicorn 기본 worker)에서 수업 피크타임에 응답 지연이 체감됨. Flask의 `Response(stream_with_context(gen()), mimetype='text/event-stream')` SSE 엔드포인트 `GET /api/rooms/<rid>/stream`을 추가해 서버가 상태 변화를 push. 클라이언트는 `new EventSource('/api/rooms/<rid>/stream')`으로 단일 연결 유지 — 연결 수는 증가하지만 불필요한 요청 수가 약 80% 감소. gunicorn에 `--worker-class gevent` 또는 `--threads 4` 옵션 추가 필요.

### 제거/단순화할 것들

- **`_compute_leaderboard()` 결과를 단기 캐시 없이 매 폴링마다 전체 재계산** (`app.py:262-290`, `app.py:1167-1175` `get_rankings()`): 순위표 API는 `/api/rooms/<rid>/rankings` 하나지만 `host_members()` (`app.py:906-912`)도 같은 함수를 호출. 학생 30명이 3초 주기로 폴링하면 초당 10회 × 5개 쿼리(members·holdings·deposits·users·join) = 50 DB 쿼리/초 발생. 룸 캐시(`_room_cache`, app.py:84-105)와 동일한 패턴으로 `_leaderboard_cache: dict = {}` + TTL 1.5초 캐시 추가, `_compute_leaderboard(rid)` 호출 전 캐시 히트 시 즉시 반환. 거래·조정·예금 생성 시 `_leaderboard_cache.pop(rid, None)` 무효화. 약 15줄 추가로 피크타임 순위표 DB 부하 90% 이상 절감.

- **`get_history()`가 가격 업데이트마다 차트 캐시 무효화 → 리로드 시 완전히 다른 히스토리 표시** (`stock_service.py:207-212`, `stock_service.py:303-332`): `get_price()` 내부에서 가격 변경 시 `for key in list(self._history_cache.keys()): if key[0] == symbol: del self._history_cache[key]` 로 차트 캐시를 지움. 다음 `get_history()` 호출 시 `random.gauss`로 완전히 새 히스토리를 생성 — 학생이 주식 상세를 열 때마다 "어제 그래프"가 달라져 "아까 올랐던 것 같았는데?" 혼란 유발. 해결: `_init_prices()` 시 각 종목의 시드 히스토리를 미리 생성해 `self._base_history: dict` 에 저장, `get_history()` 는 기준 히스토리 끝에 현재가만 보정(`bars[-1]['close'] = current`)해 반환. 가격 업데이트 시 캐시 무효화 불필요 → 차트가 일관된 과거 흐름을 보여주고 현재가만 반영.

- **`minigame_spin()` 에서 bet 파싱 `import math`가 함수 내부에서 실행** (`app.py:1379`): `import math`가 `minigame_spin()` 함수 본문 안에 위치(`app.py:1379`). Python은 이미 임포트된 모듈을 캐싱하므로 런타임 오류는 없지만 함수 호출마다 모듈 딕셔너리 조회가 발생하고, 코드 가독성 저하(파일 상단 `import math`가 없음). `app.py:6` `import os, threading, math, re, json, logging` 에 이미 `math`가 포함되어 있으므로 `app.py:1379`의 `import math` 단순 삭제.
